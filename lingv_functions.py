...
def lsi_model_create(tknz_texts, topics = 1000):
    dct = Dictionary(tknz_texts)
    #dct.add_documents([tx.split() for rule_num, tx in rules_corpus_texts]) #добавление "новых документов" - важно для правил
...
def lsi_model_update(new_texts, **kwargs):
    kwargs["dictionary"].add_documents([tx for tx in new_texts]) #добавление "новых документов" - важно для правил
    #построим корпус, состоящий из векторов, соответствующих каждому правилу:
