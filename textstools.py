...
# text_list - ['text1', 'text2', ...] - список текстов для обработки, kwargs['sinonims'] - синонимы, kwargs["ngrams"] - n-граммы, kwargs["stopwords"] - стоп-слова
...
class CollectionStatHandling(TokensHandling):
    ...
    def dictionary_corp_fill(self):
        ...
        dict_df = self.dict_frequency_df[self.dict_frequency_df['freq']>=0.0005] #частота токенов, лимитирующая количество, выбрана волюнтаристски
    ...
    def bigrams_estimate(self):
        ...
        #dfr =  pd.DataFrame(self.dict_frequency, columns = ['token', 'freq'])
...
class RulesFill(TextHandling):
    def __init__ (self, **kwargs):        
        ...
        self.dictionary = self.kwargs["dictionary"] #словарь токенизированный из коллекции (из соответствующего класса)
    ...
    def lsi_data_fill(self, topics = 500):
        ...
        try: 
            ...
            return {"dictionary" : self.dictionary, "model" : lsi_model, "rules_corpus" : rules_corpus, "num_topics" : topics}
        except:            
...
class TextsRulesApply(RulesFill):
    def __init__ (self, text, **kwargs): #m, sinonims, stopwords, ngrams, logrules, tf_parameters, tfidf_parameters, lsi_parameters, text: str)
        self.kwargs = kwargs
        self.m = kwargs["m"]
        self.text = text
        self.sinonims = kwargs['sinonims']
        self.ngrams = kwargs["ngrams"]
        self.stopwords = kwargs["stopwords"]
        self.lemm_txt = self.text_prepare(self.text) #список токенов, в которые превращается входящий (анализируемый) текст    
...
if __name__ == "__main__":        
    data_rout = r'./data'
    #data_rout = r'./data/3'
    stopwords_df = pd.read_csv(os.path.join(data_rout, 'stopwords.csv'))
    lingv_rules_df = pd.read_csv(os.path.join(data_rout, 'lingv_rules.csv'))
    sinonims_files = ['01_sinonims.csv', '02_sinonims.csv']
    
    sinonims_dfs = []
    for file_name in sinonims_files:
        sinonims_dfs.append(pd.read_csv(os.path.join(data_rout, file_name)))    

    #TokensHandling
    #переменные: kwargs["stopwords"], kwargs["sinonims"], kwargs["ngrams"]
    ngrams_df = pd.read_csv(os.path.join(data_rout, 'ngrams.csv'))
    texts_collection_df = pd.read_csv(os.path.join(data_rout, 'texts_collection.tsv'), sep = '\t')
    ...
    m = tkclass.m
    texts = ['кто должен должен сдает бухгалтерскую упрощенную отчетность с середины года физическое лицо', 'которое просто хотя ндс сдавать бурый']
    ...
    m = tkclass.m
    texts = ['кто должен должен сдает бухгалтерскую упрощенную отчетность с середины года физическое лицо счетов фактур', 'которое просто хотя ндс сдавать бурый нарушение срока в таможенную службу']
